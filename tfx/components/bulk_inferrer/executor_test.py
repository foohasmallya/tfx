...
class ExecutorTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._model_push.uri = os.path.join(self._source_data_dir,
                                        'pusher/model_push/1551926310')

    self._model_blessing = standard_artifacts.ModelBlessing()
    self._model_blessing.uri = os.path.join(self._source_data_dir,
                                            'model_validator/blessed')

    self._model_not_blessing = standard_artifacts.ModelBlessing()
    self._model_not_blessing.uri = os.path.join(self._source_data_dir,
                                                'model_validator/not_blessed')

    self._bulk_inference = standard_artifacts.BulkInference()
    self._prediction_log_dir = os.path.join(self._output_data_dir,
                                            'prediction_logs')

    self._output_destination = bulk_inferrer_pb2.BulkInferrerOutputList()
    output_list = self._output_destination.output_list.add()
    output_list.filesystem.base_directory = self._prediction_log_dir
  ...
  def _get_results(self, prediction_log_path):
    results = []
    filepattern = os.path.join(prediction_log_path,
                               executor.PREDICTION_LOGS) + '-?????-of-?????.gz'
  ...
  def testDoWithBlessedModel(self):
    ...
    output_dict = {
        'output': [self._bulk_inference],
    }
    # Create exe properties.
    exec_properties = {
        'output_destination':
            json_format.MessageToJson(self._output_destination),
        'data_spec':
    ...
    # Check outputs.
    self.assertTrue(tf.gfile.Exists(self._prediction_log_dir))
