...
class BulkInferrer(base_component.BaseComponent):
  """A TFX component to batch inference examples on given model.
  ...
  def __init__(
      ...
      output_destination: bulk_inferrer_pb2.BulkInferrerOutputList = None,
    ...
    Args:
      ...
      model_push: A Channel of 'PushedModel' type, usually produced by Pusher
        component.
      output_destination: bulk_inferrer_pb2.BulkInferrerOutputList instance that
        describes where to put inference result. _required_
      ...
      model_spec: bulk_inferrer_pb2.ModelSpec instance that describes model
        specification.
      output: Channel of `BulkInference` to store the inference results.
      instance_name: Optional name assigned to this specific instance of
        BulkInferrer. Required only if multiple BulkInferrer components are
        declared in the same pipeline.
    """
    output = output or types.Channel(
        type=standard_artifacts.BulkInference,
        artifacts=[standard_artifacts.BulkInference()])
