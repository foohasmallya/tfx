...
from hello_world.hello_component import component
...
class HelloComponentTest(tf.test.TestCase):
  ...
  def testConstruct(self):
    ...
    self.assertEqual('ExamplesPath', this_component.inputs.examples.type_name)
    artifact_collection = this_component.outputs.examples.get()
    self.assertEqual('train', artifact_collection[0].split)
    self.assertEqual('eval', artifact_collection[1].split)

    self.assertEqual('ExamplesPath', this_component.outputs.examples.type_name)
    artifact_collection = this_component.outputs.examples.get()
    self.assertEqual('train', artifact_collection[0].split)
    self.assertEqual('eval', artifact_collection[1].split)
