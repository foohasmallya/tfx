...
_pipeline_name = 'chicago_taxi_offline_inference'
...
# Path to put prediction logs. BulkInferrer will output the prediction logs
# here.
_prediction_log_dir = os.path.join(_taxi_root, 'prediction_log', _pipeline_name)
...
def _create_pipeline(pipeline_name: Text, pipeline_root: Text,
                     ...
                     module_file: Text, prediction_log_dir: Text,
  ...
  inference_example_gen = CsvExampleGen(
      input_base=inference_examples, instance_name='inference_example_gen')
  ...
  bulk_inferrer = BulkInferrer(
      ...
      output_destination=bulk_inferrer_pb2.BulkInferrerOutputList(output_list=[
          bulk_inferrer_pb2.BulkInferrerOutput(
              filesystem=bulk_inferrer_pb2.BulkInferrerOutput.Filesystem(
                  base_directory=prediction_log_dir))
      ]),
...
def main(_):
  ...
  BeamDagRunner().run(
      _create_pipeline(
          ...
          prediction_log_dir=_prediction_log_dir,
